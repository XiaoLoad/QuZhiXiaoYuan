...
**loginCode 有效期长**，可持久化到 SharedPreferences，下次启动自动恢复。
...
4. **热水器超时自动关停**：每隔 30 秒查一次 `/query/rateOrder/using`，如果 errorCode 不再是 307 则自动关闭洗澡页
5. **计时器持久化**：用 Unix 时间戳存 SharedPreferences，App 重启后计算差值恢复，不需要后台运行
...
ui/
  LoginScreen.kt     — 登录页
  MainScreen.kt      — 主页（设备列表+扫描）
  MainViewModel.kt   — 核心业务逻辑
  ShowerScreen.kt    — 洗澡中界面
  WalletScreen.kt    — 钱包+账单
  UserScreen.kt      — 用户信息+使用码
```
